...
def mfcc(signal, nfft, fs, nceps):
  ...
  # 離散コサイン変換
  ceps = scipy.fftpack.realtransforms.dct(mspec, type=2, norm="ortho", 
axis=-1)
...
if __name__ == "__main__":
  ...
  fncth=0
  for i in range(3):
    wavfile="/home/minfaox3/sence-warai/testdata/test"+str(fncth)+str(fnctw)+str(fncon)+".wav"
    nfft=2048
    nceps=12
    tmp=get_feature(wavfile,nfft,nceps)
    print(str(fncth)+str(fnctw)+str(fncon))
    print(tmp)
    tmp.append(0)
